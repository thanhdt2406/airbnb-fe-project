...
export class UserProfileComponent implements OnInit {
  ...
  userProfileForm: FormGroup = new FormGroup({
    ...
    phone_number: new FormControl(this.currentUser.phone_number, [Validators.required, Validators.minLength(10), Validators.maxLength(11)]),
  ...
  updateUserProfile(id: number) {
    const userProfile: User = {
      ...
      phone_number: this.userProfileForm.value.phone_number === "" ? this.currentUser.phone_number : this.userProfileForm.value.phone_number,
  ...
  submit() {
    if (this.selectedImages !== null) {
      const filePath = `${this.currentUser.user_name}/${this.selectedImages.name.split('.').slice(0, -1).join('.')}_${new Date().getTime()}`;

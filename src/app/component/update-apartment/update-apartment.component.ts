...
export class UpdateApartmentComponent implements OnInit {
  ...
  constructor(private imageService: ImageService,
    ...
    this.activatedRoute.paramMap.subscribe(async paramMap => {
      // @ts-ignore
      this.apartID = +paramMap.get('id');
      this.getApartment(this.apartID);
    });
  }
  ...
  async createImage() {
    const apartment = await this.updateApartment();
    if (apartment != null) {
  ...
  updateApartment() {
    ...
    if (isValidated) {
      // @ts-ignore
      return this.apartmentService.updateApartment(this.currentApartment.id, apartment).toPromise();
    }

  }

  getApartment(id: number) {
    this.apartmentService.getApartmentById(id).subscribe(value => {
      this.currentApartment = value;
      // @ts-ignore
      this.getDistrict(this.currentApartment.ward?.district?.id);
    });
  }

  getDistrict(id: number) {
    this.districtService.getDistrictById(id).subscribe(rs => {
      this.currentDistrict = rs;
      // @ts-ignore
      this.getProvince(this.currentDistrict.province?.id);
    })
  }

  getProvince(id: number) {
    // @ts-ignore
    this.provinceService.getProvinceById(id).subscribe(rs => {
      this.currentProvice = rs;

      // @ts-ignore
      this.getImageByApartment(this.currentApartment.id);
    })
  }
  ...
  getImageByApartment(id: number) {
    // @ts-ignore
    this.imageService.getAllByApartment(this.currentApartment.id).subscribe(data => {
